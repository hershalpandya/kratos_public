...
import kratos.data_io.raw_tbb_io_metadata as md
import kratos.data_io.raw_tbb_io_utilities as util

""" This module from Brian contains an awful lot of stuff. Most of it may not be that useful out of context.
Have to see how much of this we want to retain. """
...
Each station is refered to by its name (e.g. "CS001"), which is a string, or its ID (e.g. 1), which is an integer. In
...
7) LOFAR only has 96 RCUs (reciever control units) per station (at the moment).
    ...
    LOFAR keeps track of things with two ways. First, the data is all refered to by its RCUid. 0 is the 0th RCU, ect...
    ...
    should be handeld here though)

    In the code below each RCU is refered to by ANTENNA_NAME or antennaID. These are the same thing (I think). They are,
    ...
    is flipped for LBA_OUTER antenna set. X-polarization is NE-SW, and Y-polarization is NW-SE. antenna_responce.py,
...
class TBBData_Dal1:
    ...
    def __init__(
        ...
        # and use metadata
        self.ITRF_dipole_positions = md.getItrfAntennaPosition(
            self.StationName, self.antennaSet, self.metadata_dir
        )[
            self.antenna_filter
        ...
        )  # defined as callibration values in file. Never from external metadata!
    ...
    def get_station_ID(self):
        """returns the ID of the station, as an integer. This is not the same as StationName. Mapping is givin in
        utilities """
    ...
    def get_all_sample_numbers(self):
        ...
        to get time since the timestame of the first data point. Note that since these are, in general, different,
    ...
    def get_LOFAR_centered_positions(self, out=None):
        ...
        return md.convertITRFToLocal(
            self.ITRF_dipole_positions, self.metadata_dir, out=out
        )
    ...
    def get_timing_callibration_phases(self):
        ...
        phase_calibration = md.getStationPhaseCalibration(
            self.StationName,
            self.antennaSet,
            file_location=fpath,
            xMetaData_directory=self.metadata_dir,
        )
...
class MultiFile_Dal1:
    ...
    def __init__(
        ...
        # TODO: change code to make use of getters
        ...
        self.odd_pol_additional_timing_delay = (
            0.0  # anouther timing delay to add to all odd-polarized antennas
        )
    ...
    def get_LOFAR_centered_positions(self, out=None):
        ...
        md.convertITRFToLocal(
            self.get_ITRF_antenna_positions(), self.metadata_dir, out=out
        )
    ...
    def get_timing_callibration_delays(self, out=None, force_file_delays=False):
        """return the timing callibration of the anntennas, as a 1D np array. If not included in the metadata,
    ...
    def get_total_delays(self, out=None):
        ...
        station clock offsets, and trigger time offsets (nominal sample number). This function should be prefered
        over 'get_timing_callibration_delays', but the offsets can have a large average. It is recomended to pick one
        antenna (on your referance station) and use it as a referance antenna so that it has zero timing delay. Note:
        this creates two defintions of T=0. I will call 'uncorrected time' is when the result of this function is
        used as-is, and a referance antenna is not choosen. (IE, the referance station can have a large total_delay
    ...
    def get_time_from_second(self, out=None):
        ...
        accounting for delays. This is literally just the oppisite of get_total_delays """
