...
def print_table(table_header, list_of_rows):
    ...
    table_head = row_format.format(*table_entries_to_read_out)
...
want_example_3 = True
if want_example_3:
    table_entries_to_read_out = ['e.eventID', 's.stationname', 'sp.crp_selected_dipoles', 'sp.local_antenna_positions', 'pp.crp_integrated_pulse_power', 'pp.crp_integrated_noise_power']
    sql_start = "SELECT " + ', '.join(table_entries_to_read_out)
    # Gives: SELECT e.eventID, s.stationname, sp.crp_median_pulse_snr
    ...
    sql = sql_start + """ FROM events AS e
        ...
        INNER JOIN stations         AS s ON (ds.stationID=s.stationID)
        INNER JOIN stationparameters AS sp ON (s.stationID=sp.stationID)
        INNER JOIN station_polarization AS spol ON (ds.stationID=spol.stationID)
        INNER JOIN polarizations AS p ON (spol.polarizationID=p.polarizationID)
        INNER JOIN polarizationparameters AS pp ON (p.polarizationID=pp.polarizationID)
        WHERE (ed.eventID='{0}' {1} AND p.direction='{2}');""".format(eventid, station_clause, polarization)
...
want_example_4 = False
# Get LBA_INNER events with CR_FOUND and simulation_status='good simulation set'
if want_example_4:
    table_entries_to_read_out = ['e.eventID', 'e.simulation_status', 's.stationname', 's.status']
    sql_start = "SELECT " + ', '.join(table_entries_to_read_out)
    # Gives: SELECT e.eventID, s.stationname, sp.crp_median_pulse_snr
    sql = sql_start + """ FROM events AS e
        LEFT JOIN eventparameters AS ep ON (e.eventID=ep.eventID)
